...
void generatePrefix(string pat){
...
void patternMatching(string text, string pat){

    for(int i=0, j=0; i<text.size(); i++){
        while(j >=0 and pat[j+1]!= text[i]){
            j = patf[j];
        }
        if(pat[j+1] == text[i]){
            j++;
        ...
        if(j == pat.size()-1) cout << "Match at " << i - pat.size() + 2 << endl;
    }
}
...
int main()
{
   ...
   pat = "#" + pat;
   generatePrefix(pat);
   patternMatching(text, pat);

}
/// aabaabaaa

/**
    ababcababcabaabcabaabcabc
    abcabaabcabc
*/
